...
powershell -NoProfile -Command ^
  "$cb = -join ((48..57)+(97..122) | Get-Random -Count 8 | %% {[char]$_}); " ^
  "Get-ChildItem -Recurse -Filter *.html | ForEach-Object { " ^
  "  (Get-Content $_.FullName -Raw) -replace '\?r=[a-zA-Z0-9_]+', ('?r=' + $cb) | Set-Content -NoNewline $_.FullName " ^
  "}; " ^
  "$published = '%PUBLISHED_TIMESTAMP%'; " ^
  "$versionJs = \"/* Auto-generated by deploy.bat - do not edit */`r`nconst APP_PUBLISHED = '$published';`r`ndocument.addEventListener('DOMContentLoaded', function() {`r`n  document.querySelectorAll('[data-published]').forEach(function(el) {`r`n    el.textContent = 'Published ' + APP_PUBLISHED;`r`n  });`r`n});`r`n\"; " ^
