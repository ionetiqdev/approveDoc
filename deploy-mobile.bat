...
:: Same self-reference issue as promote-mobile.bat: this script checks
:: out git branches mid-run (step 0 below), and since it's a tracked
:: file, that can delete itself out from under the running process if
:: the file differs between branches. Relaunch from a temp copy first.
if not "%DEPLOY_RELAUNCHED%"=="1" (
  copy /Y "%~f0" "%TEMP%\deploy-mobile-run.bat" >nul
  set "DEPLOY_RELAUNCHED=1"
  call "%TEMP%\deploy-mobile-run.bat"
  exit /b !errorlevel!
)
