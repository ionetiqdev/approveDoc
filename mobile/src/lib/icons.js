...
/* A small colored circular chip housing an icon - the "icon chip" pattern
   used throughout the mockups (icon-search, iconChip). Role controls the
   tint: 'danger' | 'warning' | 'success' | 'accent'. */
export function iconChip(iconName, role, size = 34) {
  return `<div style="width:${size}px;height:${size}px;border-radius:${Math.round(size * 0.3)}px;background:var(--${role}-bg);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:var(--${role});">${ICONS[iconName] || ''}</div>`;
}
